<?php
    ...
    {
		public static $table 				= "ijps_tblvolume";
		public static $pkey 				= "volumeID";
		public static $messageCommonText 	= "Volume";
    	...
    	{
            $this->load->view(BACKOFFICE.'volume/addVolume');
    	}
    	
    	public function insertVolume()
        {  
            if(
                $this->input->post('txtVolumeName')!=""
            )
            {
				$prop = array( 
								'volumeName'			=>  $this->input->post('txtVolumeName', true),
                  ...
                  if($bool == 1)
                  {
					  // Add activity log start
						$prop = array( 
								'description'		=>  self::$messageCommonText." : Added (".self::$pkey." : ".$this->db->insert_id()." - Volume Name : ".$this->input->post('txtVolumeName', true).")",
								'createdByUserID'   =>  filter_var($this->session->userdata['userID'], FILTER_SANITIZE_NUMBER_INT)
							);
					...
					redirect(BACKOFFICE.SHOW_DATA_VOLUMES, 'refresh');
                  }
                  else
                  {
		...
		public function getVolume($prop)
		{	
			$volumeResult			= $this->CommonModel->getData(self::$table,array(self::$pkey=>$prop),'','','');

			if(!empty($volumeResult))
			{
				$this->load->view(BACKOFFICE.'volume/addVolume',['volumeResult' => $volumeResult]);
			}
			else
			{
        ...
        public function updateVolume()
        {
            ...
            {	
			    ...
			    $volumeID = filter_var($this->input->post('txtVolumeID'), FILTER_SANITIZE_NUMBER_INT);
                $bool = $this->CommonModel->updateRecord(self::$table, $prop, $volumeID, self::$pkey);

               if($bool == 1)
              {
				  // Add activity log start
					$prop = array( 
							'description'		=>  self::$messageCommonText." : Updated (".self::$pkey." : ".filter_var($this->input->post('txtVolumeID'), FILTER_SANITIZE_NUMBER_INT)." - Volume Name : ".$this->input->post('txtVolumeName', true).")",
							'createdByUserID'   =>  filter_var($this->session->userdata['userID'], FILTER_SANITIZE_NUMBER_INT)
						);
					$this->CommonModel->insertRecord(ACTIVITY_LOG_TABLE, $prop);
					// Add activity log end
		...
		public function setVisibilityVolume($volumeID, $isActive)
        {
            if($isActive == 1)
            {
                $isActive = 0; 
            ...
            $bool = $this->CommonModel->setVisibilityOfRecord(self::$table, $isActive, $volumeID, self::$pkey);
            
            if ($bool == 1)
            {
				// Add activity log start
					$prop = array( 
							'description'		=>  self::$messageCommonText." : Visibility Changed (".self::$pkey." : ".$volumeID." - Visibility Set As ".$isActive.")",
							'createdByUserID'   =>  filter_var($this->session->userdata['userID'], FILTER_SANITIZE_NUMBER_INT)
						);
					$this->CommonModel->insertRecord(ACTIVITY_LOG_TABLE, $prop);
					// Add activity log end
        ...
        {
            $bool    = $this->CommonModel->deleteRecord(self::$table, $volumeID, self::$pkey);
            
            if($bool == 1)
            {
				// Add activity log start
				$prop = array( 
						'description'		=>  self::$messageCommonText." : Deleted (".self::$pkey." : ".$volumeID.")",
						'createdByUserID'   =>  filter_var($this->session->userdata['userID'], FILTER_SANITIZE_NUMBER_INT)
					);
				$this->CommonModel->insertRecord(ACTIVITY_LOG_TABLE, $prop);
				// Add activity log end
